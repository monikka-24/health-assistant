...
const Profile = ({setTrigger,setName,handleSignOut}) => {
...
const navigate = useNavigate()
  ...
  useEffect(()=>{
    ...
    const getUserDetail = async ()=>{
      const res = await axios.get(`http://localhost:8080/api/users/${localStorage.getItem('id')}`)
      setUser(res.data)
  ...
  const handleDelete=async()=>{
    const res = await axios.delete(`http://localhost:8080/api/users/delete/${user.id}`)
    if(res){
      setIsEditing(true)
      setName("")
      handleSignOut()
  ...
  const handleEditSubmit = async (e) => {
    ...
    const res = await axios.put(`http://localhost:8080/api/users/put/${editForm.id}`,data)
    console.log(res)
    setUser(res.data)
