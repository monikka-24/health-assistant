...
const DoctorProfile = ({setTrigger,setName,handleSignOut}) => {
...
const navigate = useNavigate()
  ...
  useEffect(()=>{
    ...
    const getUserDetail = async ()=>{
      const res = await axios.get(`http://localhost:8080/api/doctor/${localStorage.getItem('id')}`)
      setUser(res.data)
  ...
  const handleDelete=async()=>{
    const res = await axios.delete(`http://localhost:8080/api/doctor/delete/${user.id}`)
    if(res){
      setIsEditing(true)
      setName("")
      handleSignOut()
  ...
  const handleEditSubmit = async (e) => {
    ...
    const res = await axios.put(`http://localhost:8080/api/doctor/put/${editForm.id}`,data)
    console.log(res)
    setUser(res.data)
